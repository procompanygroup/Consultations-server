    ...
    if ($validator->fails()) {
      ...
      DB::transaction(function () use ($formdata, $id) {
        ...
        if ($selectedObj->form_state == 'wait') {
          ...
          if ($pointgiftlist->count() > 0) {//يوجد هدية
//ارجاع القيمة الى جدول الهدايا
  ...
  {
    $nowsub = Carbon::now()->subDays($this->oldestday);
    $list = Selectedservice::with('answers')
      ->whereDoesntHave('service', function ($query) {
        $query->where('is_callservice', 1);
      })
    ...
    $order_ids = $list->where('form_state', 'wait')->pluck('id');
    $ordercount = $order_ids->count();

    $answer_ids = $list->where('answer_state', 'wait')->pluck('id');
    $answercount = $answer_ids->count();
    $comment_ids = $list->where('answer_state', 'agree')->whereNotNull('comment')->where('comment', '<>', '')
      ->where('comment_state', 'wait')->pluck('id');
    $commentcount = $comment_ids->count();

    $rate_ids = $list->where('answer_state', 'agree')->whereNotNull('rate_date')->where('rate_state', 'wait')->pluck('id');
    $ratecount = $rate_ids->count();
    $ids = [];
    $ids = array_merge($ids, $order_ids->toArray(), $answer_ids->toArray(), $comment_ids->toArray());
    $list = $list->whereIn('id', $ids)->sortByDesc('created_at');
    $alert_list = [];
    if ($id > 0) {
      $lastorder_row = Selectedservice::find($id);
      if ($lastorder_row) {
        $alert_list = $list->where('created_at', '>', $lastorder_row->created_at);
        $alert_list = $this->maplastorders($alert_list->take(5));
      }

    }

    $final_list = $this->maplastorders($list->take(5));
  ...
  public function maplastorders($list)
  {
    $mapList = $list->map(function ($selectedservice) {
      $type = '';
      $url = '';
      $is_comment = 0;
      if ($selectedservice->form_state == 'wait') {
        $type = 'بانتظار الموافقة';
        $url = route('order.edit', $selectedservice->id);

      } else if ($selectedservice->answer_state == 'wait') {
        $type = 'بانتظار الموافقة على الرد';
        $url = route('answer.edit', $selectedservice->id);
      } else if ($selectedservice->answer_state == 'agree' && ($selectedservice->comment_state == 'wait' || $selectedservice->rate_state == 'wait')) {
        $type = 'بانتظار الموافقة على';
        $state = [];
        if ($selectedservice->comment_state == 'wait') {
          $state[] = 'التعليق';
          $url = route('comment.edit', $selectedservice->id);
          $is_comment = 1;
        }
        ...
        $stateStr = implode(" و ", $state);
        $type = $type . ' ' . $stateStr;
      }
      $type = 'الطلب رقم ' . ' ' . $selectedservice->order_num . ' ' . $type;
      $created = Carbon::parse($selectedservice->created_at)->diffForHumans();
      return [
