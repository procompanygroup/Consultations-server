...
class GiftMinuteController extends Controller
{
  ...
  public function create()
  {
    ...
    */
    ...
    return view('admin.minutegift.create');
    ...
    if ($validator->fails()) {
      ...
      if ($is_allowed == 0) {
        ...
        DB::transaction(function () use ($side_id, $amount) {
          ...
          GiftMinute::query()->where('client_id', $side_id)->update([
            'is_active' => 0,
            'status' => 'expired',
          ]);
          $newObj = new GiftMinute;
          $newObj->client_id = $side_id;
          $newObj->free_minutes = $amount;
          $newObj->orginal_minutes = $amount;
          $newObj->is_active = 1;
...
// $pointtransfer->service_id = $expertService->service_id;
          ...
          $pointtransfer->state = 'gift-minute';
          $pointtransfer->type = $type;
          $pointtransfer->num = $newpnum;
          $pointtransfer->gift_minute_id = $newObj->id;
          $pointtransfer->save();
          //10
          $notctrlr = new NotificationController();
          $pointsval = $amount;
          $title = __('general.18addgift_title');
          $body = __('general.18addgift_body', ['Points' => $pointsval]);
  ...
  public function clientbyid($client_id)
  {
    ...
    //$newDateTime = Carbon::now()->subDays(5);
    $DBList = Client::where('is_active', 1)
      ->whereHas('giftsminutes', function ($query) use ($nowsub) {
        $query->where('is_active', 1)
          ->whereDate('created_at', '>', $nowsub);
      })
      ->with('giftsminutes', function ($query) use ($nowsub) {
        $query->where('is_active', 1)
          ->whereDate('created_at', '>', $nowsub)->select('id', 'client_id', 'free_minutes', 'created_at', 'is_active', 'orginal_minutes');
      })
      ->where('id', $client_id)
      ->select(
        'id',
  ...
  public function filltableclients()
  {
    ...
    $nowsub = Carbon::now()->subDays($days);
    //$newDateTime = Carbon::now()->subDays(5);
    $DBList = GiftMinute::with([
      'client' => function ($query) {
        $query->where('is_active', 1)
          ->select('id', 'user_name', 'mobile', 'minutes_balance', 'is_active', );
      }
    ])->select(
        'id',
        'client_id',
    ...
    $List = $DBList->map(function ($item) use ($nowsub) {
      ...
      return [
        ...
        'orginal_points' => $item->orginal_minutes,
