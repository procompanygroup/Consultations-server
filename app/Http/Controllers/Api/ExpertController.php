...
*/
    ...
    public function getwithfav()
    {
        ...
        if ($authuser->id == $id) {
            ...
            $List = $DBList ->map(function ($expert) use ($url, $recurl, $service_url, $service_icon_url) {
               //expertsServicesMap
                $expertsServicesMap = $expert->expertsServices
                ->map(function ($expertsServices) use ( $service_url, $service_icon_url) {
              ...
              // $ServiceMap1 = $expertsServices ->service->find($expertsServices->service_id)->first()->get();
                     ...
                     */
                   //  'service' =>
                     $this->servicetoArray( $expertsServices->service,$service_url, $service_icon_url);
                   // ];
...
// end expertsServicesMap
                return [
                    ...
                    'record'=>$expert->record==""?"":$recurl.$expert->record,
                    'image'=>$expert->image==""?"":$url.$expert->image,
    ...
    public function servicetoArray($service,$service_url, $service_icon_url)
    {
        return [
            "id" => $service->id,
            "name"=> $service->name,
            'desc'=> $service->desc,
            'image' =>$service->image==""?"":$service_url.$service->image,
            'icon' => $service->icon==""?"":$service_icon_url.$service->icon,
    ...
    public function getwithfavandExpServ()
    {
        ...
        if ($authuser->id == $id) {
            ...
            $List = $DBList ->map(function ($expert) use ($url, $recurl, $service_url, $service_icon_url) {
               ...
               //ServiceMap 
                    $ServiceMap = $expertsServices->service->get()->map(function ($service) use ($service_url, $service_icon_url) {
                        return [
                            'id' => $service->id,
                            'name' => $service->name,
                            'desc' => $service->desc,
